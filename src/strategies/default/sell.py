...
def sell_coins():
    debug_log("Sell coins that have reached the STOP LOSS or TAKE PROFIT threshold", False)
    """Sell coins that have reached the STOP LOSS or TAKE PROFIT threshold."""

    debug_log("Get last price", False)
    ...
    for coin in list(coins_bought):
        ...
        if last_price > TP and USE_TRAILING_STOP_LOSS:
            debug_log("Price is above the take profit and readjust SL and TP accordingly if trailing stop loss used", False)
            # increasing TP by TRAILING_TAKE_PROFIT (essentially next time to readjust SL)
            debug_log("Increasing TP by TRAILING_TAKE_PROFIT (essentially next time to readjust SL)", False)
            coins_bought[coin]['take_profit'] = price_change + TRAILING_TAKE_PROFIT
            coins_bought[coin]['stop_loss'] = coins_bought[coin]['take_profit'] - TRAILING_STOP_LOSS

            debug_log(f"{coin} TP reached, adjusting TP {coins_bought[coin]['take_profit']:.2f} and SL {coins_bought[coin]['stop_loss']:.2f} accordingly to lock-in profit", False)
            if DEBUG:
                print(
        ...
        if last_price < SL or (last_price > TP and not USE_TRAILING_STOP_LOSS):
            debug_log("The price is below the stop loss or above take profit (if trailing stop loss not used) and sell if this is the case", False)
            if last_price < buy_price:
                debug_log(f"{txcolors.SELL_LOSS}TP or SL reached, selling {coins_bought[coin]['volume']} {coin} - {buy_price} -> {last_price}: {price_change - (TRADING_FEE * 2):.2f}%{txcolors.DEFAULT}", False)
                print(
                ...
                debug_log(f"{txcolors.SELL_PROFIT}TP or SL reached, selling {coins_bought[coin]['volume']} {coin} - {buy_price} -> {last_price}: {price_change - (TRADING_FEE * 2):.2f}%{txcolors.DEFAULT}", False)
                print(
            ...
            debug_log("Try to create a real order", False)
            try:
                if not TEST_MODE:
            ...
            # error handling here in case position cannot be placed
                ...
                debug_log("Error while trying to place a real order. Error-Message: " + str(e), True)

            # run the else block if coin has been sold and create a dict for each coin sold
            else:
                debug_log("The coin has been sold", False)
        ...
        if hsp_head == 1 and len(coins_bought) == 0:
            debug_log("Not holding any coins", False)
