...
from src.config import coins_bought, client, TRAILING_TAKE_PROFIT, TRAILING_STOP_LOSS, USE_TRAILING_STOP_LOSS, LOG_TRADES, TEST_MODE, hsp_head, session_profit, DEBUG, volatility_cooloff, TRADING_FEE, QUANTITY
        ...
        if last_price < SL or last_price > TP and not USE_TRAILING_STOP_LOSS:
            if last_price < buy_price:
                ...
                # run the else block if coin has been sold and create a dict for each coin sold
                    ...
                    # Log trade
                    if LOG_TRADES:
                        profit = ((last_price - buy_price) * coins_sold[coin]['volume']) * (1 - (TRADING_FEE * 2))  # adjust for trading fee here
            ...
            if hsp_head == 1:
                if len(coins_bought) > 0:
                    print(f'TP or SL not yet reached, not selling {coin} for now {buy_price} - {last_price} : {txcolors.SELL_PROFIT if price_change >= 0. else txcolors.SELL_LOSS}{price_change - (TRADING_FEE * 2):.2f}% Est:${(QUANTITY * (price_change - (TRADING_FEE * 2))) / 100:.2f}{txcolors.DEFAULT}')
