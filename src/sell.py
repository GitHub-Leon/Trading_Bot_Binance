# This module handles the sell logic of the bot

from binance.helpers import round_step_size

# local dependencies
from src.get_price import get_price
from src.config import coins_bought, USE_TRAILING_STOP_LOSS, TRAILING_TAKE_PROFIT, TRAILING_STOP_LOSS, TESTNET, \
    LOG_TRADES, client
...
def sell_coins():
    ...
    for coin in list(coins_bought):
        ...
        if last_price < SL or last_price > TP:
            print(
                f"{txcolors.SELL}TP or SL reached, selling {coins_bought[coin]['volume']} {coin} - {buy_price} - {last_price} : {price_change:.2f}%{txcolors.DEFAULT}")
